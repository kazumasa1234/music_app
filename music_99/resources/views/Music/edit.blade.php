...
@section('content')
    ...
    <div class="position-relative vh-100">
        ...
        <div class="position-absolute top-50 start-50 translate-middle">
            <p class="text-white display-1" >EDIT</p>
        </div>
    </div>
    ...
    <div class="text-center">
        ...
        {{--        </div>--}}
        <form class="mt-5 mx-3" method="post" action="{{route('update', ['song' => $song])}}" enctype="multipart/form-data">
            @csrf
            <label for="formFile" class="form-label ">曲名</label>
            <input class="form-control form-control-lg" type="text" placeholder="曲名を入れて下さい"
                   aria-label=".form-control-lg example" name="song_name" value="{{ old('song_name', $song->song_name) }}">
            ...
            <input class="form-control form-control-lg" type="text" placeholder="アーティスト名を入れて下さい"
                   aria-label=".form-control-lg example" name="artist" value="{{ old('artist', $song->artist) }}">
            ...
            <select class="form-select form-select-lg mb-3" aria-label=".form-select-lg example"  name="category" >
                <option value="" @if(! old('category', $song->category)) selected @endif>ジャンルを選んで下さい</option>
                @foreach(config('category') as $key => $value)
                    <option value="{{ $key }}" @if(old('category', $song->category) == $key) selected @endif>{{ $value }}</option>
                @endforeach
            </select>
            ...
            <div class="mt-5">
                ...
                <input class="form-control" type="file" id="formFile" name="img_file_name" value="{{$song->img_file_name}}">
            </div>
            ...
            <div class="mt-5">
                ...
                <input class="form-control" type="file" id="formFile" name="mp3_file_name" value="{{ $song->mp3_file_name }}" >
            </div>
